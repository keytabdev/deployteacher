"use strict";
exports.id = 6999;
exports.ids = [6999];
exports.modules = {

/***/ 76999:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

// ESM COMPAT FLAG
__webpack_require__.r(__webpack_exports__);

// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "default": () => (/* binding */ Answers)
});

// EXTERNAL MODULE: external "next/dist/compiled/react/jsx-runtime"
var jsx_runtime_ = __webpack_require__(56786);
// EXTERNAL MODULE: ./components/index.ts + 3 modules
var components = __webpack_require__(57091);
// EXTERNAL MODULE: ./components/delete_table_item/DeleteTableItem.tsx
var DeleteTableItem = __webpack_require__(54698);
// EXTERNAL MODULE: ./node_modules/next/link.js
var next_link = __webpack_require__(31621);
var link_default = /*#__PURE__*/__webpack_require__.n(next_link);
// EXTERNAL MODULE: ./node_modules/react-icons/bs/index.esm.js
var index_esm = __webpack_require__(75484);
// EXTERNAL MODULE: ./node_modules/react-icons/md/index.esm.js
var md_index_esm = __webpack_require__(64348);
// EXTERNAL MODULE: ./components/loader/Loader.tsx
var Loader = __webpack_require__(51361);
// EXTERNAL MODULE: ./components/paginated_count/PaginatedCount.tsx
var PaginatedCount = __webpack_require__(84824);
// EXTERNAL MODULE: ./components/modal/Modal.tsx
var Modal = __webpack_require__(75938);
// EXTERNAL MODULE: ./app/dashboard/answers/create/CreateAnswer.tsx + 1 modules
var CreateAnswer = __webpack_require__(85162);
// EXTERNAL MODULE: ./context/useCreateAnswerContext.tsx
var useCreateAnswerContext = __webpack_require__(60721);
// EXTERNAL MODULE: ./hooks/useDebounce.ts
var useDebounce = __webpack_require__(15890);
// EXTERNAL MODULE: ./services/answer/AnswerService.ts
var AnswerService = __webpack_require__(40562);
// EXTERNAL MODULE: external "next/dist/compiled/react"
var react_ = __webpack_require__(18038);
// EXTERNAL MODULE: ./node_modules/react-hot-toast/dist/index.mjs + 1 modules
var dist = __webpack_require__(33518);
// EXTERNAL MODULE: ./node_modules/react-query/lib/index.js
var lib = __webpack_require__(53167);
// EXTERNAL MODULE: ./node_modules/qs/lib/index.js
var qs_lib = __webpack_require__(23803);
var qs_lib_default = /*#__PURE__*/__webpack_require__.n(qs_lib);
;// CONCATENATED MODULE: ./app/dashboard/answers/useAnswers.ts







function useAnswers(token, customQuery) {
    const createAnswerModalContext = (0,react_.useContext)(useCreateAnswerContext/* CreateAnswerModelContext */.f);
    const [query, setQuery] = (0,react_.useState)("");
    const [page, setPage] = (0,react_.useState)(1);
    const debounceQuery = (0,useDebounce/* default */.Z)(query, 500);
    const answersQuery = qs_lib_default().stringify({
        sort: [
            "createdAt:desc"
        ],
        pagination: {
            page: page,
            pageSize: 10
        },
        filters: {
            $or: [
                {
                    id: {
                        $eq: debounceQuery
                    }
                },
                {
                    answer_text: {
                        $containsi: debounceQuery
                    }
                },
                {
                    question: {
                        question_text: {
                            $containsi: debounceQuery
                        }
                    }
                }
            ],
            $and: [
                customQuery && {
                    question: {
                        id: {
                            $eq: customQuery
                        }
                    }
                }
            ]
        },
        fields: [
            "answer_text",
            "is_correct"
        ],
        populate: {
            question: {
                fields: [
                    "question_text"
                ]
            }
        }
    }, {
        encodeValuesOnly: true
    });
    const { data: answers, isLoading } = (0,lib.useQuery)([
        "answers",
        debounceQuery,
        page
    ], ()=>AnswerService/* default */.Z.getAnswers(token, answersQuery), {
        onError: ()=>dist/* default */.ZP.error("something went wrong please try again after refreshing")
    });
    async function deleteAnswer(id) {
        return AnswerService/* default */.Z.deleteByID(token, id);
    }
    return {
        deleteAnswer,
        answers,
        isLoading,
        query,
        createAnswerModalContext,
        setQuery,
        setPage,
        page
    };
}

;// CONCATENATED MODULE: ./app/dashboard/answers/Answers.tsx
/* __next_internal_client_entry_do_not_use__ default auto */ 










function Answers({ token, customQuery }) {
    const { deleteAnswer, answers, isLoading, query, createAnswerModalContext, setQuery, setPage, page } = useAnswers(token, customQuery);
    return /*#__PURE__*/ (0,jsx_runtime_.jsxs)(jsx_runtime_.Fragment, {
        children: [
            /*#__PURE__*/ (0,jsx_runtime_.jsxs)("div", {
                className: "w-full p-4",
                children: [
                    /*#__PURE__*/ (0,jsx_runtime_.jsxs)("div", {
                        className: "ml-auto flex w-full items-center justify-between gap-4",
                        children: [
                            /*#__PURE__*/ jsx_runtime_.jsx("h1", {
                                className: "text-2xl font-bold",
                                children: "Answers"
                            }),
                            /*#__PURE__*/ (0,jsx_runtime_.jsxs)("div", {
                                className: "flex items-center gap-4",
                                children: [
                                    /*#__PURE__*/ (0,jsx_runtime_.jsxs)("div", {
                                        className: "flex items-center gap-3 rounded-md border bg-gray-100 px-3 py-3 dark:border-gray-600 dark:bg-primaryBg",
                                        children: [
                                            /*#__PURE__*/ jsx_runtime_.jsx(index_esm/* BsSearch */.dVI, {
                                                className: "text-xl"
                                            }),
                                            /*#__PURE__*/ jsx_runtime_.jsx("input", {
                                                type: "text",
                                                className: "bg-transparent outline-none",
                                                placeholder: "search",
                                                onChange: (e)=>setQuery(e.target.value),
                                                value: query
                                            })
                                        ]
                                    }),
                                    /*#__PURE__*/ jsx_runtime_.jsx(components/* Button */.zx, {
                                        onClick: createAnswerModalContext?.onOpen,
                                        className: "flex w-fit items-center gap-2 text-white",
                                        children: /*#__PURE__*/ (0,jsx_runtime_.jsxs)(jsx_runtime_.Fragment, {
                                            children: [
                                                /*#__PURE__*/ jsx_runtime_.jsx(index_esm/* BsPlusLg */.B8K, {
                                                    className: "text-xl"
                                                }),
                                                /*#__PURE__*/ jsx_runtime_.jsx("span", {
                                                    children: "Add answer"
                                                })
                                            ]
                                        })
                                    })
                                ]
                            })
                        ]
                    }),
                    /*#__PURE__*/ (0,jsx_runtime_.jsxs)("div", {
                        className: "mt-8 rounded-md shadow-lg",
                        children: [
                            /*#__PURE__*/ jsx_runtime_.jsx("div", {
                                className: "overflow-x-auto",
                                children: /*#__PURE__*/ (0,jsx_runtime_.jsxs)("table", {
                                    className: "w-full min-w-[750px]  rounded-md border bg-white p-4 shadow-lg dark:border-none dark:bg-primaryBg",
                                    children: [
                                        /*#__PURE__*/ jsx_runtime_.jsx("thead", {
                                            className: "border dark:border-gray-600",
                                            children: /*#__PURE__*/ (0,jsx_runtime_.jsxs)("tr", {
                                                className: "text-left",
                                                children: [
                                                    /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                        className: "p-4 font-bold",
                                                        children: "ID"
                                                    }),
                                                    /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                        className: "p-4 font-bold",
                                                        children: "Question name"
                                                    }),
                                                    /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                        className: "p-4 font-bold",
                                                        children: "Answer"
                                                    }),
                                                    /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                        className: "p-4 font-bold",
                                                        children: "Is correct"
                                                    }),
                                                    /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                        className: "p-4 text-right font-bold"
                                                    })
                                                ]
                                            })
                                        }),
                                        /*#__PURE__*/ jsx_runtime_.jsx("tbody", {
                                            className: "border dark:border-gray-600",
                                            children: isLoading ? /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                colSpan: 5,
                                                className: "p-4 text-center",
                                                children: /*#__PURE__*/ jsx_runtime_.jsx(Loader/* default */.Z, {})
                                            }) : answers?.data && answers.data.length > 0 ? (answers?.data.map(({ id, attributes })=>{
                                                const { answer_text, is_correct, question } = attributes;
                                                return /*#__PURE__*/ (0,jsx_runtime_.jsxs)("tr", {
                                                    className: "dark:hover:bg-tra  w-full hover:bg-gray-100 dark:hover:bg-transparent",
                                                    children: [
                                                        /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                            className: "p-4 text-left",
                                                            children: id
                                                        }),
                                                        /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                            className: "p-4 text-left",
                                                            children: question.data?.attributes.question_text
                                                        }),
                                                        /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                            className: "p-4 text-left",
                                                            children: answer_text
                                                        }),
                                                        /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                            className: "p-4 text-left",
                                                            children: is_correct && /*#__PURE__*/ jsx_runtime_.jsx("span", {
                                                                className: "bg-green-100 text-green-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300",
                                                                children: "Correct"
                                                            })
                                                        }),
                                                        /*#__PURE__*/ (0,jsx_runtime_.jsxs)("td", {
                                                            className: "flex items-center justify-end gap-2 p-4",
                                                            children: [
                                                                /*#__PURE__*/ jsx_runtime_.jsx((link_default()), {
                                                                    href: `/dashboard/answers/${id}`,
                                                                    children: /*#__PURE__*/ jsx_runtime_.jsx(md_index_esm/* MdOutlineModeEdit */.yMw, {
                                                                        className: "cursor-pointer text-2xl text-gray-500 dark:text-white"
                                                                    })
                                                                }),
                                                                /*#__PURE__*/ jsx_runtime_.jsx(DeleteTableItem/* default */.Z, {
                                                                    fn: deleteAnswer,
                                                                    title: `${answer_text}`,
                                                                    label: "answer",
                                                                    id: id.toString(),
                                                                    keys: [
                                                                        "answers"
                                                                    ]
                                                                })
                                                            ]
                                                        })
                                                    ]
                                                }, id);
                                            })) : /*#__PURE__*/ jsx_runtime_.jsx("td", {
                                                colSpan: 4,
                                                className: "p-4 text-center",
                                                children: "No answers found"
                                            })
                                        })
                                    ]
                                })
                            }),
                            /*#__PURE__*/ jsx_runtime_.jsx(PaginatedCount/* default */.Z, {
                                page: page,
                                pageCount: answers?.meta.pagination.pageCount || 1,
                                setPage: setPage
                            })
                        ]
                    })
                ]
            }),
            /*#__PURE__*/ jsx_runtime_.jsx(Modal/* default */.Z, {
                open: createAnswerModalContext?.open,
                children: /*#__PURE__*/ jsx_runtime_.jsx(CreateAnswer["default"], {
                    token: token
                })
            })
        ]
    });
}


/***/ })

};
;